...
public class DashboardController {
    ...
    @GetMapping("/student/dashboard/deadlines")
    public ResponseEntity<BaseResponse<List<ActivityListResponse>>> getStudentDeadlines(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        log.info("GET /student/dashboard/deadlines - Fetching student deadlines with pagination: page={}, size={}", page, size);
        List<ActivityListResponse> response = dashboardService.getStudentDeadlines(page, size);
        return ResponseEntity.ok(BaseResponse.success(response, "Get student deadlines successfully"));
    }
